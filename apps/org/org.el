...
(defun hackartist/org-beamer-export-to-pdf-async ()
  (interactive)
  (let ((current-file (buffer-file-name)))
    (when (and current-file (string-equal (file-name-extension current-file) "org"))
      (save-buffer)
      (async-start
       `(lambda ()
          (require 'org)
       ...
       `(lambda (result)
          (message "exported to %s" ,current-file))))))

(defvar hackartist/org-beamer-pdf-export-timer nil
  "Org-Beamer PDF auto exporter")

(defun hackartist/start-org-beamer-pdf-export-timer (interval)
  (interactive "nInterval in seconds: ")
  (when my/org-beamer-pdf-export-timer
    (cancel-timer hackartist/org-beamer-pdf-export-timer))
  (setq hackartist/org-beamer-pdf-export-timer
        (run-at-time 0 interval #'hackartist/org-beamer-export-to-pdf-async))
  (message "export interval set to %d" interval))

(defun hackartist/stop-org-beamer-pdf-export-timer ()
  (interactive)
  (when my/org-beamer-pdf-export-timer
    (cancel-timer my/org-beamer-pdf-export-timer)
    (setq my/org-beamer-pdf-export-timer nil)
    (message "stopped exporter")))
